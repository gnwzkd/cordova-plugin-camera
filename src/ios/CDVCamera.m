...
+ (instancetype) createFromTakePictureArguments:(CDVInvokedUrlCommand*)command
{
    ...
    pictureOptions.isFinancialAlbum = [[command argumentAtIndex:12 withDefault:nil] boolValue];
    NSLog(@"pictureOptions isFinancialAlbum === %d",pictureOptions.isFinancialAlbum);
...
- (UIImage*)retrieveImage:(NSDictionary*)info options:(CDVPictureOptions*)options
{
    ...
    if (options.allowsEditing && [info objectForKey:UIImagePickerControllerEditedImage]) {
        ...
        dispatch_queue_t queue = dispatch_queue_create("assetlibrary_Queue", DISPATCH_QUEUE_SERIAL);
        
        if (options.isFinancialAlbum == true) {
            ...
            dispatch_async(queue, ^{
                [assetLibrary assetForURL:imageAssetUrl resultBlock:^(ALAsset *asset)  {
                    ...
                    [imageData writeToFile:[self GetPath:@"savePicture.jpg"] atomically:YES];
...
#pragma mark - GetPath by imageName
...
-(UIImage*)options:(CDVPictureOptions*)options image:(UIImage *)image{
    if (options.correctOrientation) {
        image = [image imageCorrectedForCaptureOrientation];
    }
...
- (void)resultForImage:(CDVPictureOptions*)options info:(NSDictionary*)info completion:(void (^)(CDVPluginResult* res))completion
{
    ...
    switch (options.destinationType) {
        case DestinationTypeNativeUri:
        {
            ...
            break;
        case DestinationTypeFileUri://相册取,拍照
        {
            image = [self retrieveImage:info options:options];
            NSData* data = nil;
            if (options.isFinancialAlbum == true) {
                data = [NSData dataWithContentsOfFile:[self GetPath:@"savePicture.jpg"]];
            }else{
